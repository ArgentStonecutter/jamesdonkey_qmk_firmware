...
__attribute__((weak)) void factory_test_indicator_kb(bool on) {}
__attribute__((weak)) void factory_test_indicator_timeup(void) {}
...
bool factory_test_indicator(void) {
    if (factory_reset_ind_state) {
        ...
        factory_test_indicator_kb(factory_reset_ind_state % 2 == 0);
        return false;
    }
...
#else
bool factory_test_indicator(void) {
    pin_t led_pins[INDICATOR_LED_COUNT] = INDICATOR_LED_PINS;
    if (factory_reset_ind_state) {
        if (factory_reset_ind_state % 2) {
            for (uint8_t i = 0; i < INDICATOR_LED_COUNT; i++) {
                writePin(led_pins[i], !INDICATORS_LED_PIN_ON_STATE);
    ...
    if (data[0] == 0xAB) {
        ...
        switch (data[1]) {
            ...
            case FACTORY_TEST_CMD_RADIO_CARRIER:
                if (data[2] < 79){
                    lkbt51_radio_test(data[2]);

                    payload[len++] = FACTORY_TEST_CMD_RADIO_CARRIER;
                    payload[len++] = data[2];
                    factory_test_send(payload, len);
                }
                break;
...
#    endif
            ...
            case FACTORY_TEST_CMD_GET_BUILD_TIME: {
                ...
                payload[len++] = '.';
                itoa((DEVICE_VER >> 0) & 0xF, (char *)&payload[len++], 16);
                payload[len++] = ' ';
